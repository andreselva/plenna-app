...
const ExpenseTable = ({ expenses = [], categories = [], creditCards = [], accounts = [], onEdit, onDelete, onPayment, onReversePayment, loading }) => {
        ...
        columns.push(
            {
                ...
                renderCell: (expense) => {
                    const creditCard = creditCards.find((cc) => Number(cc.id) === Number(expense.idCreditCard)) || {};
    ...
    columns.push({
        ...
        renderCell: (expense) => {
            const expenseActions = [
                { icon: <Pencil size={14} />, text: 'Editar', handler: () => onEdit(expense) },
                { icon: <Trash2 size={14} />, label: 'Excluir', handler: () => handleDelete(expense) }
            ];

            if (!expense.idInvoice && expense.status !== 'paid') {
                expenseActions.push({ icon: <HandCoins size={14} />, label: 'Efetuar pagamento', handler: () => onPayment(expense) });
            }
    ...
    return <FlexibleTable columns={columns} data={expenses} noDataMessage="Nenhuma despesa cadastrada" />;
};
